...
# 將 key 攤平成多層嵌套字典
def build_nested_dict(suffix, value, result_dict, lang, prefix):
    # if lang == 'zh-cn' and prefix == '':
    #     print(suffix)
    ...
    if keys:
        build_nested_dict('.'.join(keys), value, result_dict.setdefault(prefix, {}), lang, prefix)
    else:
        result_dict[prefix] = value

# 排序 key 並將 key 攤平
def sort_and_flatten_json(source_json, lang, prefix):
    ...
    for key in sorted_keys:
        ...
        if key.count('.') == 0:
            ...
            build_nested_dict(key, value, result_json, lang, prefix)
...
for lang in lang_dirs:
    for prefix in file_prefixes:
        ...
        # 排序 key 並將 key 攤平
        sorted_data = sort_and_flatten_json(data, lang, prefix)
